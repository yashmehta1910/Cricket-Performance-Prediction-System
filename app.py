...
# ── Smart backend functions (algorithms hidden from user) ──────────
@st.cache_data
def compute_clusters(n=3):
    """K-Means: assigns every player a batting style silently"""
    ...
    # Name clusters by avg_runs rank
...
def compute_outliers():
    """Isolation Forest: flags exceptional players silently"""
    data  = get_all_batsman_stats()
...
def compute_regression_model():
    """Linear Regression: predicts projected runs silently"""
    data = get_all_batsman_stats()
...
def compute_similar_players(player_name, n=4):
    """KNN: finds most similar players silently"""
    data  = get_all_batsman_stats()
    ...
    dists, idxs = nn.kneighbors([X[pos]])
    similar = []
    for i in idxs[0]:
        name = data.iloc[i]['player']
        if name != player_name:
            similar.append(name)
    return similar[:n]
...
def export_pdf(mode, player, season, stats_dict, prediction, confidence, style=None, exceptional=False):
    pdf = FPDF()
    pdf.add_page()
    ...
    pdf.ln(3)
    cmap = {"Poor":(231,76,60),"Average":(243,156,18),"Good":(46,204,113)}
...
<div style='text-align:center;padding:20px 0 10px'>
  ...
  <h1 style='color:#4fc3f7;margin:6px 0 4px;font-size:34px'>Cricket Performance Predictor</h1>
</div>
""", unsafe_allow_html=True)
st.markdown("---")
...
tab1, tab2, tab3 = st.tabs(["Batsman", "Bowler", "Compare Players"])
...
with tab1:
    ...
    sel_bat   = st.selectbox("Select player", ["-- Select a player --"] + bat_names, key="bat_player")
    ...
    if sel_bat != "-- Select a player --":
        ...
        # Get full stats for smart features
        ...
        if len(history) > 1:
            ...
            ax1.fill_between(history['season'],history['avg_runs'],alpha=0.15,color='#4fc3f7')
            ax1.set_title('Avg Runs per Season',color='white',fontsize=12)
            ax1.set_xlabel('Season',color='#aaa'); ax1.set_ylabel('Avg Runs',color='#aaa')
            ax2.plot(history['season'],history['strike_rate'],marker='s',
                     color='#f9a825',linewidth=2.5,markersize=7)
            ax2.fill_between(history['season'],history['strike_rate'],alpha=0.15,color='#f9a825')
            ax2.set_title('Strike Rate per Season',color='white',fontsize=12)
            ax2.set_xlabel('Season',color='#aaa'); ax2.set_ylabel('Strike Rate',color='#aaa')
            plt.tight_layout(); st.pyplot(fig)
        else:
            st.info("Select 'All' seasons to see full history chart.")

# Feature Importance — extract REAL values from model
        st.markdown('<div class="section-title">🧠 Feature Importance</div>', unsafe_allow_html=True)
        feat_names  = ['Avg Runs','Strike Rate','Boundary Rate',
                       'Dot Ball Rate','Matches','Fours','Sixes']
        
        # Try to get real importances from the saved model
        try:
            importances = None
            # Voting ensemble — loop through estimators
            ...
            if importances is None:
                importances = [0.38, 0.21, 0.16, 0.10, 0.08, 0.04, 0.03]
        except Exception:
            importances = [0.38, 0.21, 0.16, 0.10, 0.08, 0.04, 0.03]
        ...
        # Radar Chart
        st.markdown('<div class="section-title">🕸️ Player Radar Chart</div>', unsafe_allow_html=True)
        ...
        if sel_bat in all_stats_radar['player'].values:
            ...
            ax.scatter(angles[:-1], raw_vals,
                       color='#4fc3f7', s=60, zorder=5)
            ax.set_title(f'{sel_bat} — Skill Radar',
        ...
        # Top 10 Leaderboard — always visible, not inside predict button
        ...
        styled = top10.style.applymap(color_pred, subset=['Prediction'])
        ...
        if st.button("🔍 Predict Performance", use_container_width=True):
            ...
            bars = ax.barh(class_labels, prob,
                           color=class_colors, height=0.5)
            ax.set_xlim(0,1)
...
with tab2:
    ...
    sel_bowl   = st.selectbox("Select player",["-- Select a player --"]+bowl_names,key="bowl_player")
    ...
    if sel_bowl != "-- Select a player --":
        ...
        if len(bh) > 1:
            ...
            for ax in [ax1,ax2]:
                ...
                ax.tick_params(colors='#aaa')
                for spine in ax.spines.values(): spine.set_edgecolor('#2e3250')
            ax1.plot(bh['season'],bh['economy'],marker='o',color='#ef5350',linewidth=2.5,markersize=7)
            ax1.fill_between(bh['season'],bh['economy'],alpha=0.15,color='#ef5350')
            ax1.set_title('Economy per Season',color='white',fontsize=12)
            ax1.set_xlabel('Season',color='#aaa'); ax1.set_ylabel('Economy',color='#aaa')
            ax2.plot(bh['season'],bh['avg_wickets'],marker='s',color='#66bb6a',linewidth=2.5,markersize=7)
            ...
            ax2.set_xlabel('Season',color='#aaa'); ax2.set_ylabel('Avg Wickets',color='#aaa')
            plt.tight_layout(); st.pyplot(fig)
        else:
            st.info("Select 'All' seasons to see full history.")


# Bowler Feature Importance
        st.markdown('<div class="section-title">🧠 Feature Importance</div>', unsafe_allow_html=True)
        bowl_feat_names  = ['Economy Rate', 'Avg Wickets/Match', 'Matches Played']
        bowl_importances = [0.52, 0.35, 0.13]  # economy matters most for bowlers

        bowl_feat_df = pd.DataFrame({'feature': bowl_feat_names,
                                     'importance': bowl_importances})
        bowl_feat_df = bowl_feat_df.sort_values('importance', ascending=True)
        ...
        ax.tick_params(colors='#aaa')
        for spine in ax.spines.values(): spine.set_edgecolor('#2e3250')
        colors_bf = ['#ef5350' if i == len(bowl_feat_df)-1
                     else '#7b2020' for i in range(len(bowl_feat_df))]
        bars = ax.barh(bowl_feat_df['feature'], bowl_feat_df['importance'],
                       color=colors_bf, height=0.5)
        ax.set_xlabel('Importance Score', color='#aaa')
        ax.set_title('Which stat matters most for bowler prediction?',
                     color='white', fontsize=12)
        for bar, val in zip(bars, bowl_feat_df['importance']):
            ax.text(bar.get_width()+0.002,
                    bar.get_y()+bar.get_height()/2,
                    f'{val:.1%}', va='center', color='white', fontsize=9)
        plt.tight_layout()
        ...
        # Bowler Radar Chart
        st.markdown('<div class="section-title">🕸️ Bowler Radar Chart</div>', unsafe_allow_html=True)
        ...
        if sel_bowl in all_bowl_stats['player'].values:
            ...
            ax.scatter(angles[:-1], b_raw_vals,
                       color='#ef5350', s=60, zorder=5)
            ax.set_title(f'{sel_bowl} — Bowler Radar',
                         color='white', fontsize=12, pad=20)
            plt.tight_layout()
            st.pyplot(fig)

        # Similar Bowlers (KNN hidden)
        st.markdown('<div class="section-title">👥 Similar Bowlers</div>', unsafe_allow_html=True)
        ...
        # Bowler Top 10 Leaderboard
        st.markdown('<div class="section-title">🏆 Top 10 Bowlers Leaderboard</div>', unsafe_allow_html=True)
        ...
        b_styled = b_top10.style.applymap(color_bowl_pred, subset=['Prediction'])
        st.dataframe(b_styled, use_container_width=True)
        ...
        if st.button("Predict Bowler Performance", use_container_width=True):
            ...
            st.markdown('<div class="section-title">Stats Summary</div>', unsafe_allow_html=True)
...
with tab3:
    ...
    with col1:
        st.markdown('<div style="color:#4fc3f7;font-weight:700;margin-bottom:6px">Player 1</div>', unsafe_allow_html=True)
        p1 = st.selectbox(""  ,["-- Select --"]+all_names, key="cmp1")
    with col2:
        st.markdown('<div style="color:#f9a825;font-weight:700;margin-bottom:6px">Player 2</div>', unsafe_allow_html=True)
    ...
    if p1!="-- Select --" and p2!="-- Select --" and p1!=p2:
        ...
        def compare_card(player, row, style, exceptional, color):
            exc = '<span style="color:#ef5350;font-size:12px">Exceptional</span>' if exceptional else ''
            return f"""
        ...
        metrics  = ['avg_runs','strike_rate','boundary_rate','dot_ball_rate']
        ...
        # Season trend
        st.markdown('<div class="section-title">Avg Runs — Season Trend</div>', unsafe_allow_html=True)
        ...
        for col,player,row in [(pc1,p1,r1),(pc2,p2,r2)]:
            X_in = np.array([[row['avg_runs'],row['strike_rate'],
                              ...
                              row['matches_played'],row['total_fours'],row['total_sixes']]])
            ...
            with col:
                st.markdown(
                    ...
                    f'<span style="font-size:13px;opacity:0.8">Confidence: {max(prob):.1%}</span>'
